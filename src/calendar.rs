...
impl Calendar {
    pub fn _print_month(year: i32, month: u32) -> Result<()> {
        Self::print_month_highlight_day(year, month, 0)
    }

    pub fn print_current_month() -> Result<()> {
        let today = Local::now();
        Self::print_month_highlight_day(today.year(), today.month(), today.day() as i32)
    }

    pub fn print_month_highlight_day(year: i32, month: u32, day: i32) -> Result<()> {
        let mut rows = 0;
        let cols = 7;
        ...
        execute!(
            ...
            Print(format!(
                "{: ^20}\n",
                first_day_of_month.format("%B").to_string() + " " + &year.to_string()
            )),
